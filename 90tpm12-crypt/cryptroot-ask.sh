...
if [ $ask_passphrase -ne 0 -a -x /sbin/tpm_unlock ]; then
    luks_open="/sbin/tpm_unlock $cryptsetupopts luksOpen"
    _timeout=$(getargs "rd.luks.timeout")
    _timeout=${_timeout:-0}
    if ask_for_password --ply-tries 5 \
        --ply-cmd "$luks_open -T1 $device $luksname" \
        --ply-prompt "TPM Password ($device)" \
        --tty-tries 1 \
        --tty-prompt "TPM Password ($device)" \
        --tty-cmd "$luks_open -T5 -t $_timeout $device $luksname"; then
            ask_passphrase=0
    fi
    unset luks_open
    unset _timeout
